...
static void vSenderTask(void *pvParameters)
{
	...
	for (;; )
	{
		...
		The third parameter is the Block time ñ the time the task should be kept
		in the Blocked state to wait for space to become available on the queue
		should the queue already be full.  In this case we donít specify a block
		time because there should always be space in the queue. */
		xStatus = xQueueSendToBack(xQueue, &lValueToSend, 0);

		if (xStatus != pdPASS)
		{
			/* We could not write to the queue because it was full ñ this must
...
static void vReceiverTask(void *pvParameters)
{
	...
	for (;; )
	{
		...
		the last parameter is the block time ñ the maximum amount of time that the
